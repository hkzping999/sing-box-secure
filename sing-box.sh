...
E[115]="Quick install mode (stable VMess+WS + subscription) (sb -k)"
C[115]="极速安装模式 (稳定 VMess+WS + 订阅) (sb -l)"
...
# 安全版：生成并保存本机专用 WARP WireGuard 私钥，避免所有用户共用固定私钥
secure_generate_warp_private_key() {
  local key_file="${WORK_DIR}/warp_private_key"
...
# 安全版：内置订阅模板，避免运行时拉取第三方模板仓库
write_local_subscribe_templates() {
  mkdir -p "$TEMP_DIR"
  cat > "$TEMP_DIR/clash" <<'EOF'
mixed-port: 7890
allow-lan: true
mode: rule
log-level: warning
...
EOF
  ...
  cat > "$TEMP_DIR/sing-box-template" <<'EOF'
{
  "log": { "level": "warn" },
  ...
  "outbounds": [
    "<OUTBOUND_REPLACE>",
    { "type": "selector", "tag": "proxy", "outbounds": ["<NODE_REPLACE>", "direct"] },
...
EOF
  ...
  cat > "$TEMP_DIR/qrencode" <<'EOF'
#!/usr/bin/env bash
if command -v qrencode >/dev/null 2>&1; then
  qrencode -t ANSIUTF8 "$1"
else
  printf '[QR disabled: install qrencode to render locally] %s\n' "$1"
fi
EOF
...
check_install() {
  ...
  # 安全版：订阅模板与二维码工具全部本地生成，不访问第三方模板仓库
  write_local_subscribe_templates &
  ...
  if [ "${STATUS[0]}" = "$(text 26)" ] && [ ! -s ${WORK_DIR}/sing-box ]; then
    ...
    write_local_subscribe_templates >/dev/null 2>&1 || true
...
# 检测 IPv4 IPv6 信息（安全版：不访问第三方 IP 查询接口，仅读取本机默认路由地址）
check_system_ip() {
  local DEFAULT_LOCAL_INTERFACE4 DEFAULT_LOCAL_INTERFACE6 DEFAULT_LOCAL_IP4 DEFAULT_LOCAL_IP6
  DEFAULT_LOCAL_INTERFACE4=$(ip -4 route show default 2>/dev/null | awk '/default/ {for (i=1; i<=NF; i++) if ($i=="dev") {print $(i+1); exit}}')
  DEFAULT_LOCAL_INTERFACE6=$(ip -6 route show default 2>/dev/null | awk '/default/ {for (i=1; i<=NF; i++) if ($i=="dev") {print $(i+1); exit}}')

  if [ -n "$DEFAULT_LOCAL_INTERFACE4" ]; then
    DEFAULT_LOCAL_IP4=$(ip -4 addr show "$DEFAULT_LOCAL_INTERFACE4" 2>/dev/null | sed -n 's#.*inet \([^/ ]\+\)/[0-9]\+.*global.*#\1#gp' | sed -n '1p')
  fi
  if [ -n "$DEFAULT_LOCAL_INTERFACE6" ]; then
    DEFAULT_LOCAL_IP6=$(ip -6 addr show "$DEFAULT_LOCAL_INTERFACE6" 2>/dev/null | sed -n 's#.*inet6 \([^/ ]\+\)/[0-9]\+.*global.*#\1#gp' | sed -n '1p')
  fi

  WAN4="$DEFAULT_LOCAL_IP4"
  WAN6="$DEFAULT_LOCAL_IP6"
  COUNTRY4="local"
  COUNTRY6="local"
  EMOJI4=""
  EMOJI6=""
  ASNORG4="local-only-no-external-ip-query"
  ASNORG6="local-only-no-external-ip-query"
}
...
sing-box_variables() {
  ...
  [[ ! "${CHOOSE_PROTOCOLS,,}" =~ [b-$MAX_CHOOSE_PROTOCOLS] ]] && INSTALL_PROTOCOLS=(h) || INSTALL_PROTOCOLS=($(grep -o . <<< "$CHOOSE_PROTOCOLS" | sed "/[^b-$MAX_CHOOSE_PROTOCOLS]/d" | awk '!seen[$0]++'))
    ...
    "endpoints":[
        {
            ...
            "private_key":"$(secure_generate_warp_private_key)",
            "peers": [
              {
...
EOF
    ...
    # 生成 route 配置（安全版：保留 OpenAI 路由能力，但不再运行时下载远程 rule_set）
...
start_pre() {
    ...
    mkdir -p /var/run
    chmod 755 /var/run
    ${WORK_DIR}/sing-box check -C ${WORK_DIR}/conf"
...
"
    SING_BOX_SERVICE+="ExecStartPre=${WORK_DIR}/sing-box check -C ${WORK_DIR}/conf
...
naive+quic://${UUID[22]}:${UUID[22]}@${SERVER_IP_1}:${PORT_NAIVE}?congestion_control=bbr&security=tls&sni=${TLS_SERVER}&tls_certificate=${CERT_200_URL_1}#${NODE_NAME[22]// /%20}%20${NODE_TAG[11]}%20quic"
...

  {
    ...
    [ ! -s "$TEMP_DIR/sing-box-template" ] && write_local_subscribe_templates >/dev/null 2>&1
...
menu_setting() {
  if [[ "${STATUS[0]}" =~ $(text 27)|$(text 28) ]]; then
    ...
    ACTION[7]() { warning "Remote script execution is disabled in this security-hardened build."; exit 1; }
    ACTION[8]() { change_protocols; exit; }
    ACTION[9]() { uninstall; exit; }
    ACTION[10]() { warning "Remote script execution is disabled in this security-hardened build."; exit 1; }
    ACTION[11]() { warning "Remote script execution is disabled in this security-hardened build."; exit 1; }
    ACTION[12]() { warning "Remote script execution is disabled in this security-hardened build."; exit 1; }
  else
    OPTION[1]="1.  $(text 115)"
    ...
    ACTION[1]() { IS_FAST_INSTALL='is_fast_install'; CHOOSE_PROTOCOLS=${CHOOSE_PROTOCOLS:-'h'}; START_PORT=${START_PORT:-"$START_PORT_DEFAULT"}; CDN=${CDN:-"${CDN_DOMAIN[0]}"}; IS_SUB='is_sub'; IS_ARGO='is_argo'; HY2_PORT_HOPPING_RANGE=${HY2_PORT_HOPPING_RANGE:-'50000:51000'}; install_sing-box; export_list install; create_shortcut; exit; }
    ...
    ACTION[6]() { warning "Remote script execution is disabled in this security-hardened build."; exit 1; }
    ACTION[7]() { warning "Remote script execution is disabled in this security-hardened build."; exit 1; }
    ACTION[8]() { warning "Remote script execution is disabled in this security-hardened build."; exit 1; }
    ACTION[9]() { warning "Remote script execution is disabled in this security-hardened build."; exit 1; }
  fi
...
for z in ${!ALL_PARAMETER[@]}; do
  case "${ALL_PARAMETER[z]^^}" in
    ...
    -V )
      ...
      ;;
    -B )
      warning "Remote script execution is disabled in this security-hardened build."; exit 1
      ;;
    -R )
...
if [ "$NONINTERACTIVE_INSTALL" = 'noninteractive_install' ]; then
  ...
  CHOOSE_PROTOCOLS=${CHOOSE_PROTOCOLS:-'h'}
  START_PORT=${START_PORT:-"$START_PORT_DEFAULT"}
